...
In `@cloudflare/sandbox@next`, `exec()`, `execStream()`, and `startProcess()` are consolidated into a single `exec()` API. For more information, refer to [Migrate to Sandbox SDK next](/sandbox/guides/migrating-to-next/).
:::
