...
In `@cloudflare/sandbox@next`, hidden default sessions and the `enableDefaultSession` flag are removed. Top-level operations run sessionlessly by default. Use explicit sessions when shell state must persist. For more information, refer to [Migrate to Sandbox SDK next](/sandbox/guides/migrating-to-next/).
:::
