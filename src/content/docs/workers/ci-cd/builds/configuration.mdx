...
For preview builds, a **Preview command** runs instead (defaults to `npx wrangler preview`), which creates or updates a [Worker Preview](/workers/previews/) without promoting it to production.
...
It defaults to `npx wrangler preview`, which creates or updates a [Worker Preview](/workers/previews/) and produces a Preview URL. Like the build and deploy commands, it can be customized.

If you change this command to `npx wrangler versions upload`, the build creates a Worker version and [Version URL](/workers/configuration/version-urls/) instead of a Worker Preview. Version URLs use the resources configured for that Worker version and do not create branch-isolated resources.
