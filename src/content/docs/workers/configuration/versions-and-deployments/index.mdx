...
To test a version before deploying it, use a [Version URL](/workers/configuration/version-urls/). Version URLs use the resources configured for that Worker version. For branch-isolated testing with Preview-specific settings, use [Previews](/workers/previews/).
