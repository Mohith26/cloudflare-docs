...
| User interaction | Direct, such as chat or UI | Through Agent callbacks |

Use Agents alone for chat, messaging applications, quick API calls, real-time collaboration, and short tasks. Use Agents with Workflows for data processing pipelines, report generation, human-in-the-loop approval flows, guaranteed delivery, and multi-step operations with retry requirements.
