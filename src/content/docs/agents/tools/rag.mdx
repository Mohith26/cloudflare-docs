---
title: Retrieval Augmented Generation (with AI Search)
description: Add Retrieval Augmented Generation to Cloudflare Agents using AI Search, Vectorize, and the embedded SQL database for context retrieval.
...
Agents can use Retrieval Augmented Generation (RAG) with [AI Search](/ai-search/) to retrieve relevant information and use it to augment [calls to AI models](/agents/runtime/operations/using-ai-models/). Store a user's chat history to use as context for future conversations, summarize documents to bootstrap an Agent's knowledge base, and/or use data from your Agent's [web browsing](/agents/tools/browser/) tasks to enhance your Agent's capabilities.
