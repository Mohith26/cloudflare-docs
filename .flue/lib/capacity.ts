 ...
 * retries genuine capacity errors with exponential backoff + full jitter.
 *
 * `SkillOptions.signal` is supported in @flue/runtime 0.11.0+, so the wrapped
 * call can forward the per-attempt AbortSignal directly:
 *   session.skill("my-skill", { result: MySchema, signal })
 */
...
export class CapacityExhaustedError extends Error {
	constructor(label: string, attempts: number, cause: unknown) {
		super(`${label}: model over capacity after ${attempts} attempt(s)`, {
			cause,
		});
...
export interface CapacityRetryOptions {
	/** Human-readable label for logs/errors, e.g. "style-guide-review:123". */
	label: string;
	...
	maxDelayMs?: number;
	/** Caller cancellation signal, merged with the per-attempt timeout signal. */
	signal?: AbortSignal;
	/** Invoked before each backoff sleep — use for structured logging. */
	onRetry?: (info: {
		attempt: number;
		delayMs: number;
		error: unknown;
...
const CAPACITY_MARKERS = [
	...
	"rate limit",
	"overloaded",
	"3040", // Workers AI "Capacity temporarily exceeded" error code
];
...
function backoffDelay(
	...
	// Full jitter: random within [0, exponential] to spread retries across
	// concurrent callers all hammering the same overloaded model.
 ...
 *
 * @param fn Receives the per-attempt `AbortSignal`; forward it to the skill
 *   call: `session.skill("name", { result: Schema, signal })`.
 *   Returns a `PromiseLike` so Flue's `CallHandle` can be passed through.
 */
export async function withCapacityRetry<T>(
	...
	for (let attempt = 1; attempt <= attempts; attempt++) {
		...
		try {
			...
			// Per-attempt timeout: fail loudly, do not retry.
			if (isTimeoutAbort(error, timeoutSignal)) {
				throw new ModelTimeoutError(
					options.label,
					options.perAttemptTimeoutMs ?? 0,
					error,
				);
			}
			// Caller cancellation: propagate untouched.
			if (options.signal?.aborted) throw error;
			// Non-capacity error: not our concern, rethrow immediately.
